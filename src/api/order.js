...
export default {
    ...
    getGoodsData(data){
        return HttpClient.get('/item/for/order', data);
    },
    ...
    deleteOrderList(orderNo){ //删除订单
        return HttpClient.delete(`/replaceOrder/deleted/${orderNo}`);
    },
    //获取当前用户供应商ID
    getSupplierId(){
    ...
    getGiveawayList(data){
        return HttpClient.get('/item/gift/list/forOrder', data);
    },
    //同意退款
    ...
    //推送订单到新华书店
    pushTimeoutOrderList(data){
        return HttpClient.post(`/order/push/${data}`);
    }
