        ...
        sureBtnEvent(){
            ...
            if (this.selectTableData && this.selectTableData.length == 0) {
                ...
                for (var item of this.selectTableData){
                    ...
                    // item.specsMsg = item.skuNameCommon + '|' + item.specsMsg //后端要求添加进行处理
                    item.specsMsg = item.skuNameCommon //后端要求添加进行处理
                    item.itemSkuId = item.id
                    item.itemName = item.title
                    item.itemImg = item.mainImg
                    item.specsQuantity = 1
