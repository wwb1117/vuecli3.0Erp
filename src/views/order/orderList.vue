    ...
    <div id="orderList">
        <div class="model_topcol2" style="position: relative;">
            ...
            <div style="position: absolute; right: 20px; z-index: 999">
                <el-button v-perss="'新建代下单'" @click="addOrderListEvent" style="width: 90px" size="small">新建代下单</el-button>
            </div>
            ...
            <div class="model_content_inner">
                ...
                <div v-show="!isExportShow&&!isFreSupBoxShow" class="purchaseList_title">
                    ...
                    <!-- <div id="orderListBtnGroup">
                        ...
                        <el-button @click="orderTypeSelect($event, 0)" :style="{width: '90px'}" size="small">待付款</el-button>
                        ...
                        <table v-for="(item1, index1) in tableData" :key="index1" class="waitMergeContent_item">
                            ...
                            <tr class="table_item_title bder_bot">
                                <td colspan="9">
                                    <div style="float: left;">
                                        <el-tag v-if="item1.isReplaceOrder == 1" :style="{marginLeft: '10px'}" size="mini">代下单</el-tag>
                                    ...
                                    <div style="float: left;  margin-left: 30px">
                                        <span>下单时间 : </span>
                                        <span>{{item1.created | time_s}}</span>
                                    </div>
                                    <div style="float: right;  margin-right: 30px">
                                        <el-button style="color: #606266;" icon="fa fa-commenting" v-perss="'订单备注'" @click="addRemarkEvent(index1)" type="text">备注</el-button>
                                        <el-button style="color: #606266" v-perss="'异常处理'" icon="fa fa-exclamation-triangle" @click="addErrorEvent(index1)" type="text">异常处理</el-button>
                                    </div>
                                </td>
                            </tr>
                            <tr v-for="(item2, index2) in item1.item" :key="index2" class="table_item_goods">
                                <td class="maxWord bder_bot" style="padding-left: 10px">
                                    ...
                                    <img v-if="item2.itemImg" :src="item2.itemImg" style="float: left;width: 50px; height: 50px; margin-right: 10px">
                                    <div class="maxWord" v-text="item2.itemName"></div>
                                    <div v-if="item2.isGiving == 1" class="maxWord">
                                        <el-tag type="success" size="mini">赠品</el-tag>
                                ...
                                <td v-if="index2 == 0" :rowspan="item1.item.length" class="bder_bot center bder_left">
                                    <span v-if="item1.secondaryStatus == 0">待付款</span>
                                    ...
                                    <div v-if="item1.secondaryStatus == 1 && item1.supStatus == 1">
                                        <span class="col_cyan" v-if="item1.isConfirmOrder == 1 && item1.timeoutStatus != 2">
                                            仓库已接单
                                        </span>
                                        <span class="color_brown" v-if="item1.timeoutStatus == 2">
                                            仓库发货失败
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td style="padding: 10px" v-if="index2 == 0" class="bder_bot center bder_left" :rowspan="item1.item.length">
                                    ...
                                    <div v-if="item1.secondaryStatus == 0">
                                        <el-button v-if="item1.isReplaceOrder == 1" @click="propEvent(9, index1)" size="small" type="text">删除</el-button>
                                        <br v-if="item1.isReplaceOrder == 1">
                                        <el-button v-perss="'订单详情'" @click="propEvent(1, index1)" size="small" type="text">订单详情</el-button>
                                    </div>
                                    <!-- 待发货| -->
                                    <div v-if="item1.secondaryStatus == 1 && item1.supStatus == 1">
                                        <el-button style="width: 90px" v-if="item1.isSeparated == 0" size="small" v-perss="'发货'" @click="propEvent(3, index1)" type="primary">发货</el-button><br v-if="item1.isSeparated == 0">
                                        <el-button v-if="item1.timeoutStatus == 2 && item1.isSeparated == 0" style="margin-top: 5px" size="small" v-perss="'推送发货失败的订单到新华书店'" @click="propEvent(8, index1)" type="text">推送</el-button><br v-if="item1.timeoutStatus == 2">
                                    ...
                                    <!--
                                        注释::
                                        supStatus: 待接单,代发货状态
                                        isSeparateOrder:是不是拆出来的单（0就是原订单或者是普通单; 1就是拆出来的子订单）
                                        isSeparated: 原订单或者是普通单是否被拆分过
                                        注释::
                                    -->
                                    <!-- 待接单| -->
                                    <div v-if="item1.secondaryStatus == 1 && item1.supStatus == 0">
                                        ...
                                        <el-button v-if="item1.isSeparated == 0 && item1.isSeparateOrder == 0" size="small" v-perss="'拆分订单'" @click="propEvent(6, index1)" type="text">拆分订单</el-button><br v-if="item1.isSeparated == 0 && item1.isSeparateOrder == 0">
                                        <el-button style="margin-bottom: 5px" v-if="item1.isSeparated == 1 && item1.isSeparateOrder == 0" size="small" @click="propEvent(7, index1)" type="primary">还原订单</el-button><br v-if="item1.isSeparated == 1">
           ...
           <el-tabs v-model="activeName">
               <el-tab-pane style="border: 1px solid #e4e7ed;" v-for="(item1, index1) in logisticsInfo" :key="index1" :label="'包裹' + (index1 + 1)" :name="index1 + ''">
                   ...
                   <div style="height: 400px; overflow: auto; padding: 10px">
                       <Timeline>
                            <TimelineItem :color="index2 == 0 ? 'red' : 'blue'" v-if="item1.list" v-for="(item2, index2) in item1.list.list" :key="index2">
                                <p class="time" v-text="item2.datetime"></p>
                                <p class="content" v-text="item2.remark"></p>
                            </TimelineItem>
                       </Timeline>
...
export default {
    ...
    methods:{
        ...
        removeGiveAway(itemId){
            this.myBase.confirmDelet('你确定要永久删除此赠品?', () => {
        ...
        addLogisticsSaveBtn(){
            ...
            api.orderListSend(obj).then((response) => {
                this.$message({
                    ...
                    duration: 1500,
                    showClose: true,
                    message: '发货成功，' + response.data.info
                })
        ...
        exportOrder() {
            ...
            for (let key in this.tableParam) {
                if (this.tableParam[key] !== null && key != 'pageNo' && key != 'pageSize') {
        ...
        propEvent(type, index){
            ...
            //推送发货失败的订单到新华书店
            if (type == 8) {
                api.pushTimeoutOrderList(this.tableData[this.dataIndex].mergeOrderNo).then((response) => {
                    this.$message({
                        type: 'success',
                        duration: 1500,
                        showClose: true,
            ...
            if (type == 9) {  //待付款状态下删除订单
                this.myBase.confirmDelet('你确定要永久删除此订单信息?', () => {
                    api.deleteOrderList(this.tableData[this.dataIndex].parentOrderNo).then((response) => {
                        this.$message({
                            type: 'success',
                            duration: 1500,
                            showClose: true,
