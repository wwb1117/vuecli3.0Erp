...
export default {
    ...
    methods:{
        ...
        addGiveSaveEvent(){
            ...
            api.editGive(this.addFormData).then((response) => {
                this.$message({
                    type: 'success',
                    duration: 1500,
                    showClose: true,
        ...
        getGoodInfo(){
            ...
            api.getGoodDetail(id).then((response) => {
                ...
                for (let item1 of this.categoryListArr) {
                    if (item1.itemCategories) {
                        for (let item2 of item1.itemCategories) {
                ...
                setTimeout(() => { //需延迟,否则勾选不上
                    this.skuDetailInfo.skuList.forEach((item, index) => {
                        if (item.inventory) {
                            this.$refs.editGiveTable.toggleRowSelection(this.skuDetailInfo.skuList[index], true)
                        }
                    })
                }, 1000)
