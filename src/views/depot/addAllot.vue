            ...
            <div v-if="!isShowAddGoods" class="content" :style="{height: $store.state.home.modelContentHeight - 20 + 'px'}">
                <el-form ref="myForm" class="myForm" :inline="true" :model="addFormData" :rules="rules" label-position="right" size="small" label-width="110px">
                    ...
                    <div class="baseInfoBox">
                        <el-form-item prop="inventoryOutHouseId" label="调出仓库">
                            <el-select
                                v-model="addFormData.inventoryOutHouseId"
                                filterable
                                placeholder="请输入关键词">
                                <el-option
                        ...
                        <el-form-item prop="inventoryInHouseId" label="调入仓库">
                            <el-select
                                v-model="addFormData.inventoryInHouseId"
                                filterable
                                placeholder="请输入关键词">
                                <el-option
                                    v-for="item in houseId_option"
...
export default {
    ...
    methods:{
                    ...
                    API.searchSkuList({itemId: res.id}).then((skuList) => {
                        skuList.data.list.forEach((sku) => {
                            if (sku.skuPropertyNames == obj.itemSpec) {
                                ...
                                let obj2 = {
                                    itemId: sku.id,
                                    houseId: this.addFormData.inventoryOutHouseId,
                                    buyerId: this.$store.state.home.userInfo.user.buyerId
                                }

                                console.log(this.$store.state.home.userInfo.user.buyerId)
        ...
        save() {
            ...
            this.addFormData.buyerId = userInfo.user.buyerId
            this.addFormData.buyerName = userInfo.user.buyerCompanyName

            // 通过仓库ID和采购单ID赋值给addFormData的name
            this.houseId_option.forEach(res => {
                if (res.id === this.addFormData.inventoryOutHouseId) {
                    this.addFormData.inventoryOutHouseName  = res.warehouseName
                }
            })
            this.houseId_option.forEach(res => {
                if (res.id === this.addFormData.inventoryInHouseId) {
                    this.addFormData.inventoryInHouseName  = res.warehouseName
                }
            })
        ...
        chooseGoodEvent(){
            // 加限制
            if (!this.addFormData.inventoryOutHouseId || !this.addFormData.inventoryInHouseId) {
                this.$message({
                    type: 'warning',
                    message: '请先选择调拨仓库'
                })
