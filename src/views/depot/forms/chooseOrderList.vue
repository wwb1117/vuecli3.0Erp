<template>
    <div id="orderList">
        <div class="model_topcol" style="position: relative;">
            <span style="color: #409EFF">新增出库单</span>
            <span> - 选择订单</span>
        </div>
                            ...
                            <tr class="table_item_title bder_bot">
                                <td colspan="9">
                                    <div style="float: left; margin-left: 10px;">
                                        <el-checkbox v-model="item1.checked"  @change="chooseOrder($event, index1)"></el-checkbox>
                                    </div>
                                    <!-- <div style="float: left; margin-left: 10px;">
                                        <el-checkbox v-model="item1.checked" @change="chooseOrder($event, item1.secondaryOrderNo, key0)"></el-checkbox>
                                    </div> -->
                                    <div style="float: left; margin-left: 10px;">
                                        <el-tag v-if="item1.isReplaceOrder == 1" size="mini">代下单</el-tag>
        ...
        <div class="model_footer">
            ...
            <div style="float: left; padding-top: 10px; padding-left: 20px;">
                <el-button @click="pageSureEvent" style="width: 90px" type="primary" size="small">确定</el-button>
                <el-button @click="pageCansole" style="width: 90px" size="small">取消</el-button>
            </div>
            <el-pagination
    ...
    methods:{
        pageSureEvent(){
            ...
            if (this.selectTableData.length > 0) {
                this.$store.commit('setCurrentModelId', this.selectTableData[0])
                this.$router.push({
                    name: '新增入库',
                    params: {type: '出库', ischoose: true}
                })
            } else {
                this.$message({
                    type: 'warning',
        ...
        pageCansole(){
            ...
            this.$router.push({
                name: '新增入库',
                params: {type: '出库', ischoose: false}
            })
        },
        chooseOrder(val, index){
            this.selectTableData = []
            ...
            if (val) {
                ...
                this.selectTableData.push(this.tableData[index])
            } else {
        ...
        getTableData(){
            return dapi.getOutChooseOrderList(this.tableParam).then((response) => {
